...
if [[ "$OSTYPE" == "linux-gnu" ]]; then
    ...
    if [[ "$distro" == "arch" ]]; then
       ...
       apt install unace unrar zip unzip p7zip-full p7zip-rar sharutils rar uudeview mpack arj cabextract file-roller device-tree-compiler liblzma-dev python-pip brotli liblz4-tool gawk aria2
    fi
